...
export default class PeriodScreen extends Component {
  ...
  //No need to use state since we want to get data straight from AsyncStorage
  //and use it and the UI won't be changing
  //always use state only if there is some UI change that's going to happen
  componentDidMount() {
  ...
  async markedperiods() {
    ...
    //this.perioddate is coming from nowhere
    //I think you were supposed to use data from parsedData
    //(this the info you had saved into AsyncStorage)
    ...
    //this curly bracket I think it's not supposed to be there
    {
  ...
  getSundaysInMonth(year, month) {
    ...
    for (let i = 1; i <= new Date(year, month, 0).getDate(); i++) {
      ...
      if (date.getDay() == 0) sundays.push(moment(date).format("YYYY-MM-D"));
    }
    return sundays;
  }
            ...
            >
              ...
              >
                <TouchableOpacity
                  ...
                  style={{ backgroundColor: "#6f82c6", borderRadius: 4 }}
                >
                  <Text
                    style={{
                      textAlign: "center",
